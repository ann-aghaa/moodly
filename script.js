...
function loadEntries() {
  ...
  try {
    ...
    console.error("Could not load saved moods:", error);
    return [];
  }
}
...
function saveCurrentMood() {
  ...
  saveMessage.textContent =
    `Saved ${entry.label} for ${formatDate(date)}.`;
...
function renderHistory() {
  ...
  if (!latest.length) {
    ...
    historyList.innerHTML =
      '<p class="empty-history">No moods saved yet.</p>';
...
clearAll.addEventListener(
  ...
  () => {
    ...
    const shouldClear =
      confirm(
        "Clear all saved moods from this browser?"
      );
